...
#if 0
    {N_("tool_units"), N_("Tool size units"),
     HID_Enum, 0, 0, {0, 0, 0}, units_list, 0, 0},
#endif
};
    ...
    while ((read = gerb_fgetc(fd)) != EOF) {
        ...
        case 'M': {
            ...
            case DRILL_M_IMPERIAL:
                if (state->autod) {
                    /* BUGBUG #174 -- does not fully save user format (leading and trailing digits) */
...
drill_file_p(gerb_file_t *fd, gboolean *returnFoundBinary) {
    char* tbuf = NULL; /* locally allocated buffer */
    char* buf = NULL;  /* points within the locally allocated buffer */
    int len = 0;
    char *letter = NULL;
    int ascii = 0;
    int zero = 48; /* ascii 0 */
